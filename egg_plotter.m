...
% rectangle('Position',[x_range(1) y_range(1) width height])
% 
% hold off
...
function [delta_floor, zero] = floor_dist(fun, t, y_floor)
    ...
    delta_floor = abs(y_floor - y_range(1));
    zero = 0;
end
...
function [t_floor,t_wall] = collision_func(traj_fun, egg_params, y_floor, x_wall)
    ...
    [t_wall, wall_flag] = secant(wall_wrapper,10,10.1,dxtol,ftol,max_iter,dxmax);
    [t_floor, floor_flag] = secant(floor_wrapper,4,4.1,dxtol,ftol,max_iter,dxmax);
...
for i = 1:length(frame_time)
    ...
    [V, ~] = egg_func(s, newx, newy, new_theta, egg_params);
